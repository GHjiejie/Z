...
coding_read = require_permission(Permission.CODING_READ)
coding_approve = require_permission(Permission.CODING_APPROVE)


@router.get("/threads/{thread_id}/workspace")
def get_thread_workspace(
    thread_id: str,
    context: TenantContext = Depends(coding_read),
...
def approve_change_set(
    ...
    context: TenantContext = Depends(coding_approve),
    container=Depends(services),
):
    return container.coding.decide_change_set(
        run_id, change_set_id, True, context, payload.message, expected_version=payload.version
    )
...
def reject_change_set(
    ...
    return container.coding.decide_change_set(
        run_id, change_set_id, False, context, payload.message, expected_version=payload.version
    )
