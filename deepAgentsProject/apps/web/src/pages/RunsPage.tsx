...
export function RunsPage() {
  ...
  const load = async () => {
    setError('')
    try { setRuns((await api.runs()).items) } catch (nextError) { setError((nextError as Error).message) } finally { setLoaded(true) }
  }
  ...
  const inspect = async (id: string) => {
    ...
    } catch (nextError) { setError((nextError as Error).message) } finally { setDetailLoading(false) }
  }
  ...
  const visible = runs.filter((run) => {
    ...
    return matchesSearch && (status === 'ALL' || run.status === status) && (environment === 'ALL' || run.environment === environment)
  })
  ...
  return <div className="page-stack runs-page">
    ...
    <div className="panel runs-panel">
      ...
      <div className="table-wrap"><table className="data-table runs-table"><thead><tr><th>Run & request</th><th>Agent</th><th>Status</th><th>Attempts</th><th>Usage</th><th>Created</th></tr></thead><tbody>{visible.length ? visible.map((run) => <tr key={run.id}><td><Link className="run-cell" to={`/runs/${run.id}?${searchParams.toString()}`}><div className="mini-run-icon"><PlayCircle size={15} /></div><div><strong>{shortId(run.id)}</strong><span>{run.input.slice(0, 70)}</span></div></Link></td><td><strong className="table-strong">{run.agent_name}</strong><span className="table-sub">{run.environment}</span></td><td><StatusPill status={run.status} /></td><td><span className="icon-value"><GitBranch size={14} />{run.attempt_count ?? '—'}</span></td><td><span className="table-time">{run.usage ? `$${run.usage.cost.toFixed(4)}` : '—'}</span></td><td><time className="table-time" dateTime={run.created_at} title={new Date(run.created_at).toLocaleString()}>{formatRelative(run.created_at)}</time></td></tr>) : <tr><td colSpan={6} className="empty-cell">No runs match these filters.</td></tr>}</tbody></table></div>
    </div>

    {runId && <div className="run-detail-backdrop" onMouseDown={() => navigate(`/runs?${searchParams.toString()}`)}><aside className="run-detail" aria-label="Run inspector" onMouseDown={(event) => event.stopPropagation()}>{detailLoading && !selected ? <LoadingBlock label="Loading run trace…" /> : selected && <>
      <div className="drawer-heading"><div><span className="page-eyebrow">RUN INSPECTOR</span><h3>{shortId(selected.id)}</h3><span className="drawer-subtitle">Thread {shortId(selected.thread_id)}</span></div><div className="drawer-heading-actions"><button className="icon-button" aria-label="Copy run URL" onClick={() => navigator.clipboard.writeText(window.location.href)}><Copy size={17} /></button><button className="icon-button" aria-label="Close run inspector" onClick={() => navigate(`/runs?${searchParams.toString()}`)}><X size={18} /></button></div></div>
      ...
      <div className="run-detail-content">
        {tab === 'summary' && <RunSummary run={selected} events={events} />}
        ...
        {tab === 'usage' && <div className="usage-detail">{selected.usage ? <><div><span>Input tokens</span><strong>{selected.usage.input_tokens}</strong></div><div><span>Output tokens</span><strong>{selected.usage.output_tokens}</strong></div><div><span>Model calls</span><strong>{selected.usage.model_calls}</strong></div><div><span>Tool calls</span><strong>{selected.usage.tool_calls}</strong></div><div><span>SubAgent calls</span><strong>{selected.usage.subagent_calls}</strong></div><div><span>Estimated cost</span><strong>${selected.usage.cost.toFixed(4)}</strong></div></> : <div className="detail-empty">Usage has not been reported.</div>}</div>}
      </div>
    </>}</aside></div>}
  </div>
}
...
function RunSummary({ run, events }: { run: Run; events: RuntimeEvent[] }) {
  ...
  return <div className="run-summary-detail">
    ...
    <div className="run-detail-grid"><div><Clock3 size={15} /><span>Attempts</span><strong>{run.attempts?.length ?? 1}</strong></div><div><Activity size={15} /><span>Events</span><strong>{events.length}</strong></div><div><CircleDollarSign size={15} /><span>Cost</span><strong>${run.usage?.cost.toFixed(4) ?? '0.0000'}</strong></div><div><CalendarClock size={15} /><span>Created</span><strong title={new Date(run.created_at).toLocaleString()}>{formatRelative(run.created_at)}</strong></div></div>
