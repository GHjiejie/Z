...
export function RunsPage() {
  ...
  return <div className="page-stack">
    ...
    <div className="panel runs-panel">
      ...
      <div className="table-wrap"><table className="data-table runs-table"><thead><tr><th>Run & request</th><th>Agent</th><th>Status</th><th>Attempts</th><th>Usage</th><th>Created</th></tr></thead><tbody>{visible.length ? visible.map((run) => <tr key={run.id} onClick={() => void inspect(run)}><td><div className="run-cell"><div className="mini-run-icon"><PlayCircle size={15} /></div><div><strong>{shortId(run.id)}</strong><span>{run.input.slice(0, 70)}</span></div></div></td><td><strong className="table-strong">{run.agent_name}</strong><span className="table-sub">{run.environment}</span></td><td><StatusPill status={run.status} /></td><td><span className="icon-value"><GitBranch size={14} />{run.current_attempt_id ? '1+' : '—'}</span></td><td><span className="table-time">{run.usage ? `$${run.usage.cost.toFixed(4)}` : '—'}</span></td><td><span className="table-time">{formatRelative(run.created_at)}</span></td></tr>) : <tr><td colSpan={6} className="empty-cell">No runs match your search.</td></tr>}</tbody></table></div>
    </div>
