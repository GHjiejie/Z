...
export function CodingWorkbenchPage() {
  ...
  const start = async () => {
    ...
    try {
      const thread = await api.createCodingThread(deploymentId, title || task.slice(0, 80), repositoryId, baseRef)
  ...
  return <div className="coding-page">
    ...
    <header className="coding-launch panel">
      ...
      {!repositories.length && <div className="coding-empty-note"><GitBranch size={15} /> Register a repository through the Repository API before starting.</div>}
    </header>

    <div className="coding-workbench">
      <aside className="coding-explorer panel">
        <div className="coding-pane-head"><div><Files size={15} /><strong>Repository</strong></div><button className="icon-button" aria-label="Refresh workspace" disabled={!run} onClick={() => run && void refreshRun(run.id)}><RefreshCw size={14} /></button></div>
